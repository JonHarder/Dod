...
initState =
  let roomCryoPod = Room
        { rShortDescription = "You see the inside of a CryoPod 3001 that you are lieing down in. You think you might want to take a deeper " ++ green "look" ++ "."
      ...
      roomCryoStorage = Room
        { rShortDescription = "You are in a small room with the " ++ blue "CryoPod" ++ " that you woke up from. You should " ++ green "look" ++ " around some more to see if you can find anything else in this room."
         , rDescription = "You " ++ green "look" ++ " around the Cryo Storage room. You notice a bloody, dismemebered, " ++ blue "body" ++ " in a heap. There is also a glowing blue card " ++ blue "scanner" ++ " next to the door."
      ...
      thingCryoBody = Thing
        ...
        , tInteraction = GrabThings ("You carefully dig through the body, avoiding touching more than you have to. You find a " ++ blue "keycard" ++ " in the pockets of the body.") [thingCryoKeyCard]
        , tLabel = Label "body"
        , tRoomDescription = Nothing
        , tCombinations = Map.empty
        }
      thingCryoScanner = Thing
        { tDescription = "You see a pedestal next to the " ++ blue "door" ++ ". In the center of it's face, there is a slight indentation which is likely where a keycard would go.  Above the indentation, in bold, red lettering, you see the message \"Authorized Personelle only\""
      ...
      thingCryoKeyCard = Thing
        { tDescription = "A key card with a magnetic strip. Perhaps you could " ++ green "use" ++ " it " ++ green "on" ++ " the " ++ blue "scanner" ++ "."
      ...
      thingCryoStorageExitClosed = Thing
        ...
        , tInteraction = Inspect $ "It doesn't seem like you'll be able to force it open. You'll have to gain acess through the card " ++ blue "scanner" ++ "."
        , tLabel = Label "door"
